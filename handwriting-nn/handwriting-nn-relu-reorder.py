...
import argparse
import random
import hackthon_data_reorder as hd
import pdb
...
parser.add_argument("--classify", action="store_true", help="Classify")
parser.add_argument("--classifydata", type=str, help="Classify data")
...
if args.load is None:
  ...
  #Wxh1 = np.random.normal(0, 1, [hidden1_size, input_size]) * 0.1
  #Wh1h2 = np.random.normal(0, 1, [hidden2_size, hidden1_size]) * 0.1
  #Wh2y = np.random.normal(0, 1, [output_size, hidden2_size]) * 0.1
  Wxh1 = np.random.rand(hidden1_size, input_size)
  Wh1h2 = np.random.rand(hidden2_size, hidden1_size)
  Wh2y = np.random.rand(output_size, hidden2_size)
  #print Wxh1, Wh1h2, Wh2y
  #pdb.set_trace()
...
if args.train:
  ...
  while True:
    ...
    if epoch % 10 == 0:
      ...
      acc = float(result) / float(eval_size) * 100.0
      logging.info("Accuracy: %f%%" % (acc))

      if acc > max_acc:
        logging.info("Saving optimal model...")
        max_acc = acc 
  ...
  classify_data = hd.hackthon_data()
  classify_data.loaddata(args.classifydata)
  for i in xrange(10):
    classify_labels, classify_vectors = classify_data.getdata(1)
    logging.info("Result: %d" % classify(classify_vectors.T * input_scale))
